...
void setup() {
  ...
  for (int i = 0; i < NUM_AGENTS; i++) {
    ...
    GOALS.add(i, new Obstacle(width-GOAL_X, GOAL_Y, GOAL_RADIUS, 0, GOAL_COLOR));
    while(AGENTS.get(i).isCollidingWith(OBSTACLES)) {
      ...
      GOALS.set(i, new Obstacle(width-GOAL_X, GOAL_Y, GOAL_RADIUS, 0, GOAL_COLOR));
    }
 ...
 float time1, time2;
  if (PLANNING) {
    ...
    for (int i = 0; i < AGENTS.size(); i++) {
      ...
      time1 = millis();
      uniformCostSearch(searchGraph, AGENTS.get(i));
      time2 = millis();
      UNIFORMTIME = time2 - time1;
      
      AGENTS.get(i).path = new ArrayList<Node>();
      setupGraph(GRAPH, searchGraph, AGENTS.get(i), GOALS.get(i), OBSTACLES);
      
      time1 = millis();
      aStarSearch(searchGraph, AGENTS.get(i));
      time2 = millis();
      ASTARTIME = time2 - time1;
  ...
  fill(0);
  textSize(24);
  text("ASTAR " + ASTARTIME, 10, 20);
  text("UNIFORM " + UNIFORMTIME, 10, 50);
}
...
// --------------- UNIFORM --------------- 
boolean uniformCostSearch(ArrayList<Node> graph, Agent agent) {

  Node start = graph.get(0);
  Node goal = graph.get(1);
  
  Comparator<ArrayList<Node>> c = new CostComparatorUniform();
  PriorityQueue<ArrayList<Node>> q = new PriorityQueue<ArrayList<Node>>(11, c);
  
  start.visited = true;
  ArrayList<Node> starting = new ArrayList<Node>();
  starting.add(start);
  q.add(starting);
 
  while (q.size() > 0) {
    ArrayList<Node> p = q.poll();
    
    Node m = p.get(p.size() - 1);
    if (m == goal) {
      for (int i = 0; i < p.size(); i++) {
        agent.path.add(i, p.get(i));
      }
      return true;
    }
    for(int i = 0; i < m.neighbors.size(); i++) {
...
float calcPriority(ArrayList<Node> p) {
  float sum = 0;
  for (int i = 0; i < p.size() - 1; i++) {sum += PVector.dist(p.get(i).pos, p.get(i+1).pos); }
  return sum;
}

public class CostComparatorUniform implements Comparator<ArrayList<Node>>
{
...
class Agent {
  ...
  void update(float dt, ArrayList<Obstacle> obs, ArrayList<Agent> agents) {
    ...
    acc.add(sForce.mult(3));
    acc.add(cForce.mult(0.5));
